...
- **Дубликаты workflow в логах n8n** (`Activated workflow "chat-orchestrator"` дважды): удалите лишние в UI или перезапустите n8n с образом `v3-purge-dupes` (entrypoint чистит по имени перед import). Иначе старый webhook с `lastNode` держит HTTP ~15–30 с и ai падает по timeout.
